...
public:
  ...
  explicit Navi2LocationServer(): Node("navi2_location_server")
  {
    // ACTION
    this->n2p_client =
    ...
    this->navi2_srv = this->create_service<NavLoc>(
      ...
      std::bind(&Navi2LocationServer::searchLocationName, this, _1, _2) );
    // PUB
  ...
  void sendGoalPose(const std::vector<double> coord_list) {
    ...
    goal_options.goal_response_callback =
      std::bind(&Navi2LocationServer::naviResponseCB, this, _1);
    goal_options.feedback_callback = 
      std::bind(&Navi2LocationServer::naviFeedbackCB, this, _1, _2);
    goal_options.result_callback = 
      std::bind(&Navi2LocationServer::naviResultCB, this, _1);
...
void execute(){
  ...
  auto node = rclcpp::Node::make_shared<Navi2LocationServer>(); //type: std::shared_ptr<rclcpp::Node>
  rclcpp::WallRate rate(333ms);
  Navi2LocationServer nas;
  while (rclcpp::ok()) {
